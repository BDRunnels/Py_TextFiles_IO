...
def next_invoice_number(invoice_number: str) -> str:
    ...
    """
    inv_year, number = parse_invoice_number(invoice_number)
    current = get_year()
    if current == inv_year:
        number += 1
    else:
        inv_year = current
        number = 1
    new_invoice_number = f"{inv_year}-{number:04d}"
...
def record_invoice(invoice_file: TextIO,
    ...
    :param last_line_pointer: The position of the start of the last line
        in the file. This will be obtained by the previous call to `record_invoice`
    :return int representing position file pointer is at (reading / writing purposes)
        last line in file. This can be used in subsequent calls to `record_invoice`
    """
    invoice_file.seek(last_line_pointer, SEEK_SET)  #  defaults to start of file before trying to read lines
    last_row = ""
    ...
    if last_row:
        ...
        invoice_number = last_row.split("\t")[0]  # grabbing only item needed from invoice style (invoice number)
        new_invoice_number = next_invoice_number(invoice_number)
    else: # if file is empty, we'll start numbering at 1
        year = get_year()
